...
export default async function AdminProjectEditorPage({ params, searchParams }: ProjectEditorPageProps) {
  const [{ id }, query] = await Promise.all([params, searchParams])

  const [project, mediaAssets] = await Promise.all([getProjectForAdmin(id), getMediaForAdmin()])
  ...
  return (
    <div className="space-y-4">
      ...
      {error ? (
        <ActionToast
          type="error"
          message={error === "create_failed" ? "Could not create project." : "Could not save project."}
        />
      ) : null}
      <Card className="rounded-2xl border-slate-900/10 dark:border-white/10">
        <CardHeader>
          <CardTitle className="font-mono text-xl">{id === "new" ? "New Project" : "Edit Project"}</CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
          ...
          {error ? (
            <p className="rounded-md border border-destructive/30 bg-destructive/10 px-3 py-2 text-sm text-destructive">
              {error === "create_failed" ? "Could not create project." : "Could not save project."}
            </p>
          ) : null}
          <ProjectForm
            project={
              project
                ? {
                    ...
                    tech_tags: Array.isArray(project.tech_tags)
                      ? project.tech_tags.filter((item: unknown): item is string => typeof item === "string")
                      : [],
